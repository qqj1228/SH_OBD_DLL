    ...
    public class OBDDeviceELM327 : OBDDevice {
        ...
        public override bool Initialize(Settings settings) {
            ...
            if (settings.ComPort > 0) {
                // 使用串口连接ELM327
                m_log.TraceInfo(string.Format("Attempting initialization on port {0}", settings.HardwareIndex.ToString()));
                return Initialize(settings.ComPort, settings.BaudRate);
            } else {
                // 使用TCP连接ELM327
                m_log.TraceInfo(string.Format("Attempting initialization on remote server {0}:{1}", settings.RemoteIP, settings.RemotePort.ToString()));
